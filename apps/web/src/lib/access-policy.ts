...
export function isSameOrigin(request: Request) {
  ...
  const allowed = [new URL(expected).origin, ...(process.env.AUTH_ALLOWED_ORIGINS || '').split(',').filter(Boolean)];
...
export function accessSnapshot(row: Record<string, unknown>) {
  return Object.fromEntries(['id', 'full_name', 'is_active', 'role', 'roles', 'permissions', 'name', 'session_version']
