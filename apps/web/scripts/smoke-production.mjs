...
function session(user) {
  const payload = Buffer.from(JSON.stringify({ id: user.id, email: user.email,
    fullName: user.full_name, role: user.role, sessionVersion: user.session_version,
...
try {
  ...
  for (const route of ['/api/internal/governance', '/api/internal/audit', '/api/internal/lios/health', '/api/internal/export', '/api/internal/monthly-report']) {
  ...
  for (const [name, id] of identities) {
    ...
    for (const route of ['/acesso/dashboard', '/acesso/governanca', '/acesso/lios', '/api/internal/governance', '/api/internal/audit', '/api/internal/lios/health', '/api/internal/lios/applications']) {
  ...
  assert.ok(ordinary, 'existing non-privileged identity');
  const cookie = session(ordinary);
  for (const route of ['/api/internal/governance', '/api/internal/audit', '/api/internal/lios/health']) {
    assert.equal((await request(route, cookie)).status, 403, route);
  }
  assert.equal((await request('/api/internal/governance', cookie, { method: 'POST',
    headers: { origin: base, 'Content-Type': 'application/json' }, body: '{}' })).status, 403);
  assert.equal((await request('/acesso/governanca', cookie)).status, 307);
  pass('ordinary user denied in real pages and APIs, including mutations');
  ...
  assert.ok(migrations.includes('007_governance.sql') && migrations.includes('008_lios.sql'));
  ...
  if (supabase.status !== 'reachable') console.log(`BLOCKED: external Supabase authentication (${supabase.code}); session/RBAC checks passed independently.`);
  const result = { sha, at: new Date().toISOString(), status: supabase.status === 'reachable' ? 'passed' : 'passed_with_external_blocker',
    ...
    passwordLogin: 'tested in isolated database; Supabase simulated; personal passwords unchanged' };
